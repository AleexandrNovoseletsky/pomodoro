...
class CRUDRepository[T]:
    ...
    def __init__(self, sessionmaker: async_sessionmaker, orm_model: type[Any]):
        ...
        connectivity     orm_model: SQLAlchemy model class that this
        repository manages
        """
    ...
    async def create_object(self, data: BaseModel) -> T:
        ...
        Args:     data: Pydantic schema containing field values for
        creation

        Returns:     Created model instance with database-generated
        fields

        Note:     Automatically refreshes the instance to include
    ...
    async def get_all_objects(self) -> list[T]:
        ...
        Note:     Returns empty list if no records exist in the table
        """
        async with self.sessionmaker() as session:
    ...
    async def update_object(
        ...
        Args:     object_id: Primary key identifier of the object to
        update     update_data: Pydantic schema containing fields to
        modify

        Returns:     Updated model instance if found, None if object
        doesn't exist

        Note:     Uses exclude_unset=True to only update provided
    ...
    async def delete_object(self, object_id: int) -> bool:
        ...
        Returns:     True if object was successfully deleted, False if
        object didn't exist

        Note:     This is a hard delete operation that permanently
        removes the record.     Consider soft deletion using is_active
